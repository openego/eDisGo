...
def extract_optimised_charging_timeseries(grid_id):
    ...
    reference_charging_ts = pd.read_csv(
        edisgo_dir + r"\charging_points_active_power.csv", index_col=0, parse_dates=True
    )
    x_charge_ev_grid = combine_results_for_grid(
        feeders, grid_id, res_dir, "x_charge_ev"
    )
    reference_charging_ts.update(x_charge_ev_grid)
    os.makedirs(edisgo_dir_new, exist_ok=True)
    reference_charging_ts.loc[
        reference_charging_ts.index, reference_charging_ts.columns
    ].to_csv(edisgo_dir_new + r"\charging_points_active_power.csv")
