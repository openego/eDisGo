...
bus_voltage = (
    combine_results_for_grid([feeder_id], grid_id, results_base_dir, "v_bus") / 20
)
...
losses = pd.concat(
    [
        pypsa_grid.lines_t.p0 + pypsa_grid.lines_t.p1,
        pypsa_grid.transformers_t.p0 + pypsa_grid.transformers_t.p1,
    ],
    axis=1,
)
losses_q = pd.concat(
    [
        pypsa_grid.lines_t.q0 + pypsa_grid.lines_t.q1,
        pypsa_grid.transformers_t.q0 + pypsa_grid.transformers_t.q1,
    ],
    axis=1,
)

branch_active_power_acpf = pd.concat(
    [
        (pypsa_grid.lines_t.p0 - pypsa_grid.lines_t.p1) / 2,
        (pypsa_grid.transformers_t.p0 - pypsa_grid.transformers_t.p1) / 2,
    ],
    axis=1,
)
branch_reactive_power_acpf = pd.concat(
    [
        (pypsa_grid.lines_t.q0 - pypsa_grid.lines_t.q1) / 2,
        (pypsa_grid.transformers_t.q0 - pypsa_grid.transformers_t.q1) / 2,
    ],
    axis=1,
)
