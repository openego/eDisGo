...
def _build_component_timeseries(
    ...
    if kind == "gen":
        p_set2 = (psa_net.generators_t.p_set[disp_gens]).round(3)
        q_set2 = (psa_net.generators_t.q_set[disp_gens]).round(3)
        p_set = (
            pd.concat(
                [
                    p_set2,
        ...
        q_set = (psa_net.generators_t.q_set[p_set.columns] / s_base).round(3)
        for comp in p_set.columns:
            comp_i = _mapping(
                psa_net,
                edisgo_obj,
        ...
        if len(flexible_loads) > 0:
            p_set = (edisgo_obj.dsm.p_max[flexible_loads] / s_base).round(3)
            p_min = (edisgo_obj.dsm.p_min[flexible_loads] / s_base).round(3)
            e_min = (edisgo_obj.dsm.e_min[flexible_loads] / s_base).round(3)
            e_max = (edisgo_obj.dsm.e_max[flexible_loads] / s_base).round(3)
