...
def _validate_etrago_specs(network):
    ...
    if set(capacity_edisgo.keys()).intersection(set(capacity_etrago.columns)):
        for gen_type in capacity_edisgo.keys():
            if not isclose(capacity_edisgo[gen_type], capacity_etrago[gen_type],
                           rel_tol=1e-3):
                logger.error("Capacity in edisgo for generator type '{}' does "
                             "not match capacity given in ETraGoSpecs.".format(
                                 gen_type))
    else:
        gen_types_missing = list(set(capacity_edisgo.keys()) -
    ...
    if set(load_edisgo.keys()).intersection(set(load_etrago.columns)):
        for load_type in load_edisgo.keys():
            if not isclose(load_edisgo[load_type], capacity_etrago[gen_type],
                           rel_tol=1e-3):
                logger.error("Capacity in edisgo for generator type '{}' does "
                             "not match capacity given in ETraGoSpecs.".format(
                                 gen_type))
