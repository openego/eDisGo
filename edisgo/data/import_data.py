...
def _validate_etrago_specs(network):
    ...
    def _compare_etrago_edisgo_values(edisgo_dict, etrago_df):
        # compare entries found in edisgo and etrago
        for t in set(edisgo_dict.keys()).intersection(set(etrago_df.columns)):
            if not isclose(edisgo_dict[t], etrago_df[t].values[0],
                           rel_tol=1e-3):
                logger.error(
                    "ETraGo specifications for '{}' do not match eDisGo "
                    "value (ETraGo = {} vs. eDisGo = {}).".format(
                        t, etrago_df[t].values[0], edisgo_dict[t]))
        # check for entries only found in edisgo or etrago
        types_missing = (list(set(edisgo_dict.keys()) -
                              set(etrago_df.columns)) +
                         list(set(etrago_df.columns) -
                              set(edisgo_dict.keys())))
        for t in types_missing:
            if t in edisgo_dict.keys():
                if edisgo_dict[t] != 0:
                    logger.error(
                        "ETraGo specifications for '{}' do not match "
                        "eDisGo value (ETraGo = / vs. eDisGo = {}).".format(
                            t, edisgo_dict[t]))
            else:
                if etrago_df[t].values[0] != 0:
                    logger.error(
                        "ETraGo specifications for '{}' do not match "
                        "eDisGo value (ETraGo = {} vs. eDisGo = /).".format(
                            t, etrago_df[t].values[0]))
    ...
    load_etrago['retail'] = (load_etrago.iloc[0].get('retail', 0) +
                             load_etrago.iloc[0].get('industrial', 0))
    try:
        del load_etrago['industrial']
    except:
        pass
