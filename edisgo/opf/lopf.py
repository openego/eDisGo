    ...
    **kwargs,
    ...
    if fixed_parameters["optimize_emob"]:
        ...
        if any([i.empty for i in fixed_parameters["ev_flex_bands"].values()]):
            ...
            logger.warning("Emob optimization is set to False as flex bands empty.")
        else:
            if not fixed_parameters["flexible_loads"].empty:
                fixed_parameters["optimized_charging_points"] = (
                    fixed_parameters["flexible_loads"]
    ...
    if fixed_parameters["optimize_hp"]:
        ...
        if fixed_parameters["optimized_heat_pumps"].empty:
            ...
            logger.warning("HP optimization is set to False as optimized hps empty.")
    else:
        # Add empty list to later define inflexible loads
        fixed_parameters["optimized_heat_pumps"] = []
    ...
    **kwargs,
    ...
    kwargs :
        ...
        energy_level_starts : dict('ev':pd.Series, 'tes':pd.Series)
            ...
            Not really necessary anymore. As initial timestep starts with 50%
        charging_starts : dict('ev':pd.Series, 'tes':pd.Series)
            dict('ev':None, 'tes':None, 'hp':None)
            initial charging value of component (first timestep)
        energy_level_ends : dict('ev': bool/pd.Series, 'tes':bool/pd.Series)
            If True, final energy level is 50% capacity
            (default: dict("ev": None, "tes": None)
    ...
    model.objective_name = objective
    ...
    if fixed_parameters["optimize_emob"]:
        ...
        model = add_ev_model_bands(
            ...
            energy_level_ends=kwargs.get("energy_level_ends", {"ev": None}),
    ...
    if fixed_parameters["optimize_hp"]:
        ...
        model = add_heat_pump_model(
            ...
            energy_level_ends=kwargs.get("energy_level_ends", {"tes": None}),
...
def add_ev_model_bands(
    ...
    energy_level_ends={"ev": None},
    energy_level_beginning=None,
    charging_starts={"ev": None},
):
    """
    Method to add sets, variables and constraints for including EV flexibility
    ...
    energy_level_ends : dict('ev':bool/pd.Series)
    ...
    model = add_rolling_horizon(
        ...
        energy_level_ends=energy_level_ends,
...
def add_rolling_horizon(
    ...
    for energy_attr in energy_attrs[comp_type.lower()]:
        ...
        setattr(
            ...
            pm.Param(
                flex_set,
                initialize=energy_level_ends[energy_attr],
                mutable=True,
                within=pm.Any,
            ),
        )
...
def add_heat_pump_model(
    ...
    fixed_parameters :
    energy_level_starts : dict('ev':pd.Series, 'tes':pd.Series)
    energy_level_ends : dict('ev':bool/pd.Series, 'tes':floatbool/pd.Series
    energy_level_beginning :
    charging_starts : dict('ev':pd.Series, 'tes':pd.Series, 'hp':pd.Series)
    ...
    def charging_tes(model, hp, time):
        ...
        return model.energy_level_tes[hp, time] == (
            1 - 0.04 * pd.to_timedelta(model.time_increment) / pd.to_timedelta("24h")
...
def optimize(model, solver, load_solutions=True, mode=None, tee=True, **kwargs):
    ...
    if (results.solver.status == SolverStatus.ok) and (
        ...
        if hasattr(model, "flexible_heat_pumps_set"):
            ...
            result_dict["slack_initial_charging_tes"] = pd.Series(
                model.slack_initial_charging_pos_tes.extract_values()
            ) + pd.Series(model.slack_initial_charging_neg_tes.extract_values())
...
def aggregated_energy(model, time):
    """
    Constraint aggregating the power of bess and emob and hp charging
