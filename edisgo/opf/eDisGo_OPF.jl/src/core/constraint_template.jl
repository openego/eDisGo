...
function constraint_power_balance_bf(pm::AbstractBFModelEdisgo, i::Int; nw::Int=nw_id_default)
    ...
    bus_gens_slack = ref(pm, nw, :bus_gens_slack, i)
    ...
    branch_strg_pf = Dict(k => ref(pm, nw, :branch, k, "storage_pf") for k in bus_lines_to)

    bus_pg = Dict(k => ref(pm, nw, :gen, k, "pg") for k in bus_gens)
    bus_qg = Dict(k => ref(pm, nw, :gen, k, "qg") for k in bus_gens)

    bus_pg_nd = Dict(k => ref(pm, nw, :gen_nd, k, "pg") for k in bus_gens_nd)
    bus_qg_nd = Dict(k => ref(pm, nw, :gen_nd, k, "qg") for k in bus_gens_nd)

    bus_pd = Dict(k => ref(pm, nw, :load, k, "pd") for k in bus_loads)
    bus_qd = Dict(k => ref(pm, nw, :load, k, "qd") for k in bus_loads)

    bus_storage_pf = Dict(k => tan(acos(ref(pm, nw, :storage, k, "pf")))*ref(pm, nw, :storage, k, "sign") for k in bus_storage)
