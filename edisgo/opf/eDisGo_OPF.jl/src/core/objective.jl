function objective_min_line_loading(pm::AbstractBFModelEdisgo)
    ...
    l = Dict(n =>
        ...
        for n in nws)
    c = Dict(n =>
            Dict(i => get(branch, "cost", 1.0) for (i,branch) in ref(pm, n, :branch))
        for n in nws)
    s_nom = Dict(n =>
    ...
    pgs = Dict(n => var(pm, n, :pgs) for n in nws)
...
function objective_min_losses_slacks(pm::AbstractBFModelEdisgo)
    ...
    return JuMP.@objective(pm.model, Min,
        ...
        #+ factor_ll * sum(sum(p[n][(b,i,j)]/s_nom[n][b] for (b,i,j) in ref(pm, n, :arcs_from)) for n in nws)  # minimize line loading
