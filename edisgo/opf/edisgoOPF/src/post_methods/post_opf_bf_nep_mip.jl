...
&&&\\newline
&1\\leq n_{ik} \\leq \\boldsymbol{N_{max}}, \\quad n_{ik} \\in \\mathbb{Z}&&\\forall \\left(i,k \\right)  \\in E
\\end{aligned}
\\end{equation} ``
"""
function post_opf_bf_nep_mip(pm::GenericPowerModel{T};maxexp::Integer=10,obj::String="both") where T <: PowerModels.AbstractBFForm
    # cost factor for network expansion variable
    costfactor = 10
    # add variables for network expansion
    add_var_ne(pm,maxexp=maxexp)
