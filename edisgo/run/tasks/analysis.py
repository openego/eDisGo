...
def task_base_reinforce(
    ...
    ctx : RunContext
        ...
        ``ctx.flags['base_reinforce_artifact']`` on save.
    ...
    if save_artifact and ctx.results_dir is not None:
        ...
        ctx.flags["base_reinforce_artifact"] = artifact_dir + ".zip"
...
def task_optimize(
    ...
    *,
    ...
    Which assets are flexible defaults to the config's top-level
    ``flexibilities:`` list — the same list ``import_flex`` imports —
    so one config key controls both. The ``flexible`` shortcut expands
    to the lower-level ``flexible_*`` lists automatically:

    * ``"heat_pumps"`` → all loads of type ``heat_pump``
    * ``"electromobility"`` / ``"charging_points"`` → all loads of
      type ``charging_point``
    * ``"home_batteries"`` / ``"storage"`` → all storage-unit indices
    * ``"dsm"`` → all DSM-ready load indices
    ...
    ctx : RunContext
        ...
        ``flexible`` is not given. The resolved ``flexible_*`` name lists are
    ...
    flexible : list of str, optional
        ...
        "electromobility", "charging_points", "home_batteries",
        "storage", "dsm"}``. Defaults to the config's
        ``flexibilities:`` list; pass an empty list to optimize
        nothing implicitly.
    ...
    """
    if flexible is None:
        flexible = ctx.raw_config.get("flexibilities") or []
    # accept the flexibilities: carrier names alongside the OPF-level ones
    flexible = [
        {"electromobility": "charging_points", "home_batteries": "storage"}.get(f, f)
        for f in flexible
    ]
