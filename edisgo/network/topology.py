...
class Topology:
    ...
    def add_heat_pump(self, bus, p_set, type="heat_pump", **kwargs):
        """
        Adds heatpump to topology.

        Heatpump name is generated automatically.
        ...
        --------
        str
            Unique identifier of added heatpump.
        ...
        # generate heatpump name and check uniqueness
        if bus_s.lv_grid_id is not None and not np.isnan(bus_s.lv_grid_id):
            grid = self.get_lv_grid(int(bus_s.lv_grid_id))
        else:
            grid = self.mv_grid
        ...
        heatpump_name = f"{tmp}_{load_id}"

        if heatpump_name in self.loads_df.index:
            random.seed(a=int(load_id))

            while heatpump_name in self.loads_df.index:
                heatpump_name = f"{tmp}_{random.randint(10**8, 10**9)}"
        ...
        return heatpump_name

    def add_charging_point(self, bus, p_set, type="charging_point", **kwargs):
        """
        Adds charging point to topology.

        Charging point name is generated automatically.
        ...
        --------
        str
            Unique identifier of added charging point.
        ...
        charging_point_name = f"{tmp}_{load_id}"

        if charging_point_name in self.loads_df.index:
            random.seed(a=int(load_id))

            while charging_point_name in self.loads_df.index:
                charging_point_name = f"{tmp}_{random.randint(10 ** 8, 10 ** 9)}"
