...
def select_cable_14a( #CHANGED#14a
    edisgo_obj: EDisGo,
    level: str,
    apparent_power: float,
    component_type: str | None = None,
    ...
    if component_type == "charging_point":
        try:
            ...
            hits = cp_df[abs(cp_df["apparent_power_kVA"] - s_kva) < 1e-3]
            if not hits.empty:
                row = hits.iloc[0]
                print("\n=== CHARGING POINT DEBUG ===")
                print("Charging point id:", row.name)
                print("Park id:", row.get("charging_park_id"))
                print("Use case:", row.get("use_case"))
                print("P [kW]:", row.get("p_nominal_kW"))
                print("S [kVA]:", row.get("apparent_power_kVA"))
                print("Geometry:", row.get("geometry"))
                print("============================\n")
