...
def pypsa2ppc(psa_net):
    ...
    try:
        ...
        print("Dictionary for load timeseries of timehorizon {} created".format(time_horizon))
    except IndexError as e:
        print("No load timeseries. Create empty dicts "
              "for timeseries of load")
        load_dict = dict()
    try:
        gen_dict = _build_generator_dict(psa_net, ppc)
        print("Dictionary for generator timeseries of timehorizon {} created".format(time_horizon))
    except IndexError as e:
        print("no generator timeseries Create empty dicts "
...
def ppc2pm(ppc,psa_net): #pragma: no cover
    ...
    pm = {"gen": dict(), "branch": dict(), "bus": dict(), "dcline": dict(), "load": dict(), "storage": dict(),
          ...
          "sourcetype": "matpower", "per_unit": True, "name": ppc["name"]}
    load_idx = 1
    shunt_idx = 1
    for row in ppc["bus"]:
        bus = dict()
...
def _build_bus(psa_net,ppc):
    ...
    col_names = "index","type","Pd","Qd","Gs","Bs","area","v_mag_pu_set",\
    ...
    ppc["bus"][:, VMAX] = [min(val,1.05) for val in psa_net.buses["v_mag_pu_max"].values]
    ppc["bus"][:, VMIN] = [max(val, 0.985) for val in psa_net.buses["v_mag_pu_min"].values]
    return
...
def _build_branch(psa_net,ppc):
    ...
    from_bus = np.array([psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.lines["bus0"]])
    to_bus = np.array([psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.lines["bus1"]])
...
def _build_transformers(psa_net,ppc):
    ...
    from_bus = np.array([psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.transformers["bus0"]])
    to_bus = np.array([psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.transformers["bus1"]])
...
def _build_load(psa_net,ppc):
    ...
    load_buses = np.array([psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.loads["bus"]])
    print("build {} loads, distributed on {} buses".format(n_load,len(np.unique(load_buses))))
...
def _build_generator_dict(psa_net,ppc):
    ...
    gen_buses = [psa_net.buses.index.get_loc(bus_name) for bus_name in psa_net.generators["bus"]]
    for t in range(time_horizon):
        ...
        for (gen_idx, bus_idx) in enumerate(gen_buses):
            ...
            if np.isnan(qg):
                qg = 99999
            generator_dict["gen_data"][str(t+1)][str(gen_idx + 1)] = {"pg": pg, "qg": qg, "gen_bus": int(bus_idx)+1,
                                                                "status": True, "index": int(gen_idx + 1)}
