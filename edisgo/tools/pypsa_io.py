    ...
    * Duplicate labels in components DataFrames and components' time series
    ...
    mode : str
        ...
        * 'mvlv' to export MV network level only. This includes cumulative load
          ...
          # ToDo change name of this mode or use kwarg to define where to
             aggregate lv loads and generation
        ...
        for lv_grid in grid_object.lv_grids:
            if mode == "mv":
                ...
                if len(lv_grid.transformers_df.bus1.unique()) > 1:
                    raise Exception(
                        "More than one station for lv grid {} "
...
def _append_lv_components(
    ...
    if comp == "Load":
        ...
        if aggregate_generators is None:
            ...
            comps_aggr = comps.groupby("type").sum().loc[:, ["bus", "control", "p_nom"]]
            comps_aggr.bus = bus
    ...
    for lv_grid in network.mv_grid.lv_grids:
        ...
        for gen in lv_grid.generators:
            ...
            generation[gen.type][gen.subtype]["timeseries_p"].append(
                gen.pypsa_timeseries("p").rename(gen_name).to_frame().loc[timesteps]
            )
            generation[gen.type][gen.subtype]["timeseries_q"].append(
                gen.pypsa_timeseries("q").rename(gen_name).to_frame().loc[timesteps]
            )
        ...
        for lo in lv_grid.graph.nodes_by_attribute("load"):
            for sector, val in lo.consumption.items():
                ...
                load[sector]["timeseries_p"].append(
                    lo.pypsa_timeseries("p").rename(repr(lo)).to_frame().loc[timesteps]
                )
                load[sector]["timeseries_q"].append(
                    lo.pypsa_timeseries("q").rename(repr(lo)).to_frame().loc[timesteps]
                )
...
def _check_topology(components):
    ...
    line_buses = (
        components["Line"]["bus0"].tolist() + components["Line"]["bus1"].tolist()
    )
    ...
    buses_to_check = line_buses + load_buses + generator_buses + transformer_buses
    ...
    if missing_buses:
        raise ValueError("Buses {buses} are not defined.".format(buses=missing_buses))
...
def _check_integrity_of_pypsa(pypsa_network):
    ...
    time series for active and reactive power, and completeness of buses and branch
...
def process_pfa_results(edisgo, pypsa, timesteps):
    ...
    edisgo.results._i_res = np.hypot(pypsa.lines_t["p0"], pypsa.lines_t["q0"]).truediv(
    ...
    edisgo.results.pfa_v_ang_seed = (
        pd.concat(
            [edisgo.results.pfa_v_ang_seed, pypsa.buses_t["v_ang"].loc[timesteps, :]]
        )
        .reset_index()
        .drop_duplicates(subset="index", keep="last")
        .set_index("index")
