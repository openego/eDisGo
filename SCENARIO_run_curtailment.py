...
def _overwrite_edisgo_timeseries(edisgo, pypsa_network):
    ...
    if not edisgo.topology.charging_points_df.empty:
        charging_points = [_ for _ in pypsa_network.loads.index
...
def _curtail(pypsa_network, gens, loads, time_steps, elia_logger):
    ...
    ts_res_load_zero = residual_load[residual_load == 0.].index
...
def _calculate_curtailed_energy_incl_reactive(pypsa_network_orig, pypsa_network):
    ...
    curtailed_load_reactive_ts = (
            pypsa_network_orig.loads_t.q_set -
            pypsa_network.loads_t.q_set
    )
    return curtailed_feedin_ts, curtailed_feedin_reactive_ts, \
           curtailed_load_ts, curtailed_load_reactive_ts
...
def solve_convergence_issues(
        edisgo, pypsa_network, timesteps,
    ...
    for timestep in timesteps:
        ...
        elia_logger.debug(
            "Time step with convergence issue: {}".format(timestep))
        ...
        while converged is False and iteration_count < max_iterations:
            ...
            elia_logger.debug(
                "Iteration round: {}".format(iteration_count))
            ...
            angle_diff = pd.Series(
                pypsa_network.buses_t.v_ang.loc[
                    timestep, pypsa_network.lines.bus0].values -
                pypsa_network.buses_t.v_ang.loc[
                    timestep, pypsa_network.lines.bus1].values,
                index=pypsa_network.lines.index)
            angle_diff.sort_values(ascending=False, inplace=True)

            feeder = edisgo.topology.lines_df.at[
                angle_diff.index[0], "mv_feeder"]
            ...
            pf_results = pypsa_network.pf(
                timestep, use_seed=True)

            # check if power flow converged
            tmp = pf_results["converged"][
    ...
    curtailment.at[
        "convergence_issues_mv", "feed-in"] += curtailed_feedin.sum().sum()
    curtailment.at[
        "convergence_issues_mv", "load"] += curtailed_load.sum().sum()
...
def solve_convergence_issues_reinforce(
    ...
    curtailed_load, curtailed_load_reactive = \
        _calculate_curtailed_energy_incl_reactive(
            pypsa_network_orig, pypsa_network)
    curtailment.at[
        "convergence_issues_mv", "feed-in"] += curtailed_feedin.sum().sum()
    curtailment.at[
    ...
    return curtailed_feedin, curtailed_feedin_reactive, curtailed_load, \
           curtailed_load_reactive, curtailment, voltage_dev, rel_load
...
def solve_convergence_issues_lv_grid(
    ...
    curtailed_feedin, curtailed_load = _calculate_curtailed_energy(
        pypsa_network_orig, pypsa_network)
    curtailment.at[
        "convergence_issues_lv", "feed-in"] += curtailed_feedin.sum().sum()
    curtailment.at[
        "convergence_issues_lv", "load"] += curtailed_load.sum().sum()
        ...
        while len(time_steps_issues) > 0 and iteration_count < max_iterations:
            ...
            for feeder in feeders:
                ...
                ts_issues = voltage_issues.loc[
                            :, buses[buses.isin(voltage_issues.columns)]].dropna(how="all").index
            ...
            if len(timesteps_not_converged) > 0:
                curtailment, voltage_dev, rel_load = \
                    solve_convergence_issues_lv_grid(
...
def curtailment_lv_voltage_single_lv_grid(
        edisgo, pypsa_network, curtailment, voltage_dev, rel_load,
        grid_results_dir, timesteps_not_converged):
    elia_logger = logging.getLogger(
        'elia_project: {}'.format(edisgo.topology.id))
    ...
    voltage_issues = voltage_dev_grid[
        voltage_dev_grid != 0.].dropna(how="all").dropna(
        axis=1, how="all")
    buses_issues = voltage_issues.columns
    time_steps_issues = voltage_issues.index.append(
        timesteps_not_converged).unique()
        ...
        while len(time_steps_issues) > 0 and iteration_count < max_iterations:
            ...
            for feeder in feeders:
                ...
                buses = pypsa_network.buses[
                    pypsa_network.buses.lv_feeder == feeder].index

                gens_feeder = pypsa_network.generators[
                    pypsa_network.generators.bus.isin(buses)].index
                loads_feeder = pypsa_network.loads[
                    pypsa_network.loads.bus.isin(buses)].index
...
def curtailment_mvlv_stations_voltage(
    ...
    elia_logger = logging.getLogger(
        'elia_project: {}'.format(edisgo.topology.id))
    elia_logger.info("Start curtailment due to voltage issues at MV/LV "
                     "stations.")
...
def curtailment_mv_voltage_10_percent(
    ...
    voltage_issues = v_mag_pu_pfa[
        (v_mag_pu_pfa > 1.1) | (v_mag_pu_pfa < 0.9)].dropna(
        how="all").dropna(axis=1, how="all")
    time_steps_issues = voltage_issues.index
    buses_issues = voltage_issues.columns
...
def curtailment_lv_lines_overloading_single_lv_grid(
    ...
    overloading_issues = rel_load_grid[rel_load_grid > 1.].dropna(
        how="all").dropna(axis=1, how="all")
    lines_issues = overloading_issues.columns
    ...
    if len(time_steps_issues) > 0:
        ...
        while len(time_steps_issues) > 0 and iteration_count < max_iterations:
            ...
            # get feeders with overloading issues
            # get all buses with issues
            buses_issues = pypsa_network.lines.loc[
                lines_issues, ["bus0", "bus1"]].stack().unique()
            buses_df_issues = buses_df.loc[buses_issues, :]
            feeders = buses_df_issues.loc[:, "lv_feeder"].dropna().unique()
            ...
            for feeder in feeders:
                ...
                gens_feeder = pypsa_network.generators[
                    pypsa_network.generators.bus.isin(
                        buses_downstream)].index
                loads_feeder = pypsa_network.loads[
                    pypsa_network.loads.bus.isin(
                        buses_downstream)].index
...
def curtailment_mvlv_stations_overloading(
    ...
    elia_logger = logging.getLogger(
        'elia_project: {}'.format(edisgo.topology.id))
    elia_logger.info("Start curtailment due to overloading issues at MV/LV "
                     "stations.")
    ...
    stations_secondary_sides = {
        _: "BusBar_mvgd_{}_lvgd_{}_LV".format(
            edisgo.topology.id, _.split("_")[-1])
...
def curtailment_mv_lines_overloading(
    ...
    overloading_issues = rel_load_mv[rel_load_mv > 0.98].dropna( #Todo: Why 0.98?
        how="all").dropna(axis=1, how="all")
    lines_issues = overloading_issues.columns
    time_steps_issues = overloading_issues.index
...
def curtailment_hvmv_station_overloading(
    ...
    elia_logger = logging.getLogger(
        'elia_project: {}'.format(edisgo.topology.id))
    elia_logger.info("Start curtailment due to overloading issues of the "
                     "HV/MV station.")
...
def curtail_lv_grids(
        edisgo, curtailment, timesteps_not_converged, grid_results_dir,
        voltage_dev, rel_load):
    elia_logger = logging.getLogger(
        'elia_project: {}'.format(edisgo.topology.id))
    elia_logger.debug(
        "Number of time steps with convergence issues: {}".format(
    ...
    time_steps_issues_lv = voltage_issues.index.append(
        overloading_issues.index).unique()
    ...
    if len(timesteps_not_converged) > 0:
        ...
        buses_issues = edisgo.topology.lines_df.loc[
            overloading_issues.columns, ["bus0", "bus1"]].stack().unique()
        lv_grids_overloading_issues = edisgo.topology.buses_df.loc[
            buses_issues, "lv_grid_id"]
        # get set of LV grids with issues
        lv_grids = lv_grids_voltage_issues.append(
            lv_grids_overloading_issues).unique()

    elia_logger.info(
        "Number of LV grids to curtail: {}".format(
            len(lv_grids)
        )
    )
    ...
    for lv_grid_id in lv_grids:
        ...
        pypsa_lv.buses = pypsa_lv.buses.join(
            edisgo.topology.buses_df.loc[
            :, ["lv_feeder", "path_length_to_station"]])
        pypsa_lv.lines = pypsa_lv.lines.join(
            edisgo.topology.lines_df.loc[:, ["lv_feeder"]])

        curtailment, voltage_dev, rel_load = \
            curtailment_lv_voltage_single_lv_grid(
                edisgo, pypsa_lv, curtailment, voltage_dev, rel_load,
                grid_results_dir, timesteps_not_converged)
...
def calculate_curtailment(variation, mode="mv_lv"):
    ...
    while remaining_issues and i < max_iterations:
        ...
        elia_logger.info(
            ...
            "{}: {}.".format(
                i, len(time_steps_issues)))

        if mode != "mv":
            # curtail LV grids
            curtailment, voltage_dev, rel_load = curtail_lv_grids(
                edisgo, curtailment, timesteps_not_converged,
                grid_dir, voltage_dev, rel_load)

        # curtailment MV-LV stations
        curtailment, voltage_dev, rel_load = \
            curtailment_mvlv_stations_overloading(
                edisgo, curtailment, voltage_dev, rel_load, grid_dir)
        curtailment, voltage_dev, rel_load = \
            curtailment_mvlv_stations_voltage(
                edisgo, curtailment, voltage_dev, rel_load, grid_dir)
        ...
        curtailment, voltage_dev, rel_load = curtailment_mv_voltage_10_percent(
            edisgo, curtailment, voltage_dev, rel_load, grid_dir)
        curtailment, voltage_dev, rel_load = \
            curtailment_mv_lines_overloading(
                edisgo, curtailment, voltage_dev, rel_load, grid_dir)
        ...
        elia_logger.info(
            "Check for remaining issues after round {}.".format(i)
        )
        remaining_issues = False
        edisgo.analyze(timesteps=time_steps_issues,
                       mode=pypsa_mode[mode])
        ...
        tol = 1e-4
        voltage_issues = voltage_dev[
            abs(voltage_dev) > tol].dropna(
            how="all").dropna(axis=1, how="all")
        v_mag_pu_pfa = edisgo.results.v_res
        voltage_issues_10_percent = v_mag_pu_pfa[
            (v_mag_pu_pfa > 1.1) | (v_mag_pu_pfa < 0.9)].dropna(
            how="all").dropna(axis=1, how="all")
        ...
        tol = 1e-4
        overloading_issues = rel_load[
            rel_load > 1. + tol].dropna(
            how="all").dropna(axis=1, how="all")
        ...
        # to those time steps
        time_steps_issues = voltage_issues.index.append(
            overloading_issues.index).append(
            voltage_issues_10_percent.index).unique()
    ...
    if remaining_issues:
        ...
        if not voltage_issues_10_percent.empty:
            voltage_issues.to_csv(
                os.path.join(
                    grid_dir, "remaining_voltage_issues_10_percent.csv"))
        if not overloading_issues.empty:
            overloading_issues.to_csv(
                os.path.join(
                    grid_dir, "remaining_overloading_issues.csv"))
    else:
        elia_logger.info("Success. All issues solved.")

    # save curtailment sums
    curtailment.to_csv(
        os.path.join(grid_dir, "curtailment_01percent.csv"))
