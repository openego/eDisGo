...
class Test_get_component_timeseries:
    ...
    def test_timeseries_imported(self):
        ...
        msg = (
            "Columns or indices of inserted storage timeseries do not match "
    ...
    def test_import_load_timeseries(self):
        ...
        load = timeseries.import_load_timeseries(
    ...
    def test_worst_case(self):
        ...
        exp = pd.Series(
            data=[0.1 * 0.001209, 1.0 * 0.001209],
            name=load,
            index=self.timeseries.timeindex,
        )
        assert_series_equal(
        ...
        assert_series_equal(
            self.timeseries.storage_units_active_power.loc[:, storage],
            exp,
            check_exact=False,
            check_dtype=False,
        )
        ...
        exp = pd.Series(
            data=[0.1 * 0.001222], name=load, index=self.timeseries.timeindex
        )
        assert_series_equal(
            self.timeseries.loads_active_power.loc[:, load],
            exp,
        ...
        load = "Load_retail_LVGrid_9_14"  # industrial, lv
        exp = pd.Series(
            data=[1.0 * 0.001222], name=load, index=self.timeseries.timeindex
        )
        assert_series_equal(
            self.timeseries.loads_active_power.loc[:, load],
            exp,
    ...
    def test_add_loads_timeseries(self):
        ...
        active_power_new_load = self.timeseries.loads_active_power.loc[
        ...
        reactive_power = self.timeseries.loads_reactive_power[load_name]
        assert np.isclose(
            active_power.iloc[4], (4.150392788534633e-05 * annual_consumption)
        )
        assert np.isclose(
            reactive_power.iloc[13],
            (7.937985538711569e-05 * annual_consumption * tan(acos(0.9))),
        )
    ...
    def test_add_generators_timeseries(self):
        ...
        p_nom = 1.7
        gen_name = self.topology.add_generator(
            generator_id=5,
            p_nom=p_nom,
        ...
        timeseries.add_generators_timeseries(
            self,
            gen_name,
            generators_active_power=new_gen_active_power,
        ...
        assert np.isclose(
            self.timeseries.generators_reactive_power.loc[timeindex, gen_name],
            p_nom * 0.5,
        ).all()
        # add multiple generators and check
        p_nom2 = 1.3
        ...
        new_gens_active_power = pd.DataFrame(
            index=timeindex,
            columns=[gen_name2, gen_name3],
            data=(
                np.array([[p_nom2 * 0.97], [p_nom3 * 0.98]])
                .repeat(len(timeindex), axis=1)
                .T
            ),
        )
        new_gens_reactive_power = pd.DataFrame(
            ...
            columns=[gen_name2, gen_name3],
            data=(
                np.array([[p_nom2 * 0.5], [p_nom3 * 0.4]])
                .repeat(len(timeindex), axis=1)
                .T
            ),
        )
        ...
        new_gens_reactive_power = pd.DataFrame(
            ...
            columns=[gen_name2, gen_name3],
            data=(
                np.array([[p_nom2 * 0.54], [p_nom3 * 0.45]])
                .repeat(len(timeindex), axis=1)
                .T
            ),
        )
    ...
    def test_add_storage_unit_timeseries(self):
        ...
        assert np.isclose(
            np.array(
                self.timeseries.storage_units_active_power.loc[
                    timeindex, [storage_name2, storage_name3]
                ].values,
                dtype=float,
            ),
        ...
        timeseries.add_storage_units_timeseries(
            self,
            storage_name,
            storage_units_active_power=new_storage_active_power,
        ...
        new_storages_active_power = pd.DataFrame(
            index=timeindex,
            columns=[storage_name2, storage_name3],
            data=(
                np.array([[p_nom2 * 0.97], [p_nom3 * 0.98]])
                .repeat(len(timeindex), axis=1)
                .T
        ...
        timeseries.add_storage_units_timeseries(
            self,
            storage_name,
            timeseries_storage_units=new_storage_active_power,
        ...
        timeseries.add_storage_units_timeseries(
            ...
            [storage_name2, storage_name3],
            timeseries_storage_units=new_storages_active_power,
            timeseries_storage_units_reactive_power=new_storages_reactive_power,
        )
        assert self.timeseries.storage_units_active_power.shape == (
            24,
            num_storage_units + 3,
    ...
    def test_check_timeseries_for_index_and_cols(self):
        ...
        timeseries_with_wrong_timeindex = pd.DataFrame(
            index=timeindex[0:12],
            columns=added_comps,
            data=np.random.rand(12, len(added_comps)),
        )
        # Todo: check what happens with assertion. Why are strings not the same?
        msg = (
            "Inserted timeseries for the following components have the a "
            "wrong time index:"
        )
        with pytest.raises(ValueError, match=msg):
            timeseries.check_timeseries_for_index_and_cols(
                self, timeseries_with_wrong_timeindex, added_comps
            )
        timeseries_with_wrong_comp_names = pd.DataFrame(
            index=timeindex, columns=["Comp_1"], data=np.random.rand(13, 1)
        )
        msg = (
        ...
        with pytest.raises(ValueError, match=msg):
            timeseries.check_timeseries_for_index_and_cols(
                self, timeseries_with_wrong_comp_names, added_comps
            )
    ...
    def create_random_timeseries_for_topology(self, timeindex):
        ...
        generators_active_power = pd.DataFrame(
            ...
            data=np.multiply(
                np.random.rand(len(timeindex), len(generator_names)),
                ([self.topology.generators_df.p_nom] * len(timeindex)),
            ),
        )
        generators_reactive_power = pd.DataFrame(
            index=timeindex,
        ...
        storage_units_active_power = pd.DataFrame(
            index=timeindex,
            columns=storage_names,
            data=np.multiply(
                np.random.rand(len(timeindex), len(storage_names)),
                ([self.topology.storage_units_df.p_nom] * len(timeindex)),
            ),
        )
        storage_units_reactive_power = pd.DataFrame(
            index=timeindex,
            columns=storage_names,
            data=np.multiply(
                np.random.rand(len(timeindex), len(storage_names)),
