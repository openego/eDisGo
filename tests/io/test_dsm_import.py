...
class TestDSMImport:
    ...
    def test_oedb(self, oep_engine):
        ...
        dsm_profiles = dsm_import.oedb(
            edisgo_object, scenario="eGon2035", engine=oep_engine
        )
    ...
    @pytest.mark.oep
    def test_get_profiles_per_industrial_load(self, oep_engine):
        ...
        dsm_profiles = dsm_import.get_profiles_per_industrial_load(
            load_ids=[], scenario="eGon2035", engine=oep_engine
        )
        assert dsm_profiles["p_min"].empty

    @pytest.mark.oep
    def test_get_profile_cts(self, oep_engine):
        edisgo = EDisGo(
            ding0_grid=pytest.ding0_test_network_3_path, legacy_ding0_grids=False
        )
