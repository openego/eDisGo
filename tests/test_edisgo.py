...
class TestEDisGo:
    ...
    def test_crit_station(self):
        ...
        assert (
            overloaded_lv_station.at["LVGrid_1", "time_index"]
            == self.timesteps[1]
        )
        assert np.isclose(
        ...
        assert (
            overloaded_lv_station.at["LVGrid_4", "time_index"]
            == self.timesteps[0]
        )
    ...
    def test_analyze(self):
        ...
        assert np.isclose(
            self.edisgo.results.v_res.loc[
                self.timesteps, "Bus_BranchTee_LVGrid_4_2"
            ].values,
        ...
        assert np.isclose(
            self.edisgo.results.v_res.loc[
                self.timesteps, "virtual_BusBar_MVGrid_1_LVGrid_4_MV"
            ].values,
        ...
        assert np.isclose(
            self.edisgo.results.pfa_p.loc[
                self.timesteps, "Line_60000002"
            ].values,
            np.array([0.00801, 0.08144]),
            atol=1e-5,
        ).all()
        assert np.isclose(
            ...
            np.array([0.00263, 0.02661]),
            atol=1e-5,
        ).all()
        assert np.isclose(
            self.edisgo.results.i_res.loc[
                self.timesteps, ["Line_10005", "Line_90000021"]
            ].values,
    ...
    def test_mv_to_pypsa(self):
        ...
        assert np.isclose(
            pypsa_network.generators_t["p_set"].loc[
                self.timesteps, "LVGrid_1_fluctuating"
            ],
            [0.04845, 0],
        ).all()
        assert np.isclose(
            pypsa_network.loads_t["p_set"].loc[
                self.timesteps, "LVGrid_1_agricultural"
            ],
            [0.01569, 0.1569],
        ).all()
    ...
    def test_mv_lv_to_pypsa_with_charging_points(self):
        ...
        cp1 = self.edisgo.add_component(
            ...
            ts_active_power=pd.Series(
                data=np.array([0.01, 0.02]), index=self.timesteps
            ),
            ts_reactive_power=pd.Series(
                data=np.array([0.04, 0.03]), index=self.timesteps
            ),
        ...
        cp2 = self.edisgo.add_component(
            comp_type="ChargingPoint",
            ts_active_power=pd.Series(
                data=np.array([0.05, 0.06]), index=self.timesteps
            ),
            ts_reactive_power=pd.Series(
                data=np.array([0.08, 0.07]), index=self.timesteps
            ),
            bus="BusBar_MVGrid_1_LVGrid_2_LV",
            p_nom=0.005,
            use_case="work",
    ...
    def test_edisgo_timeseries_analysis(self):
        ...
        ts_storage = pd.DataFrame({"Storage_1": [0.0] * 8760}, index=timeindex)
        edisgo = EDisGo(
            ding0_grid=test_network_directory,
    ...
    def test_add_component(self):
        ...
        assert (
            self.edisgo.topology.lines_df.loc[line_name, "bus0"]
            == "Bus_MVStation_1"
        )
        assert (
            self.edisgo.topology.lines_df.loc[line_name, "bus1"] == "Testbus"
        )
        ...
        assert (
            self.edisgo.topology.loads_df.loc[load_name, "annual_consumption"]
            == 3.2
        )
        assert (
            self.edisgo.topology.loads_df.loc[load_name, "sector"]
            == "residential"
        )
        ...
        assert (
            self.edisgo.topology.generators_df.loc[gen_name, "bus"]
            == "Testbus"
        )
        assert self.edisgo.topology.generators_df.loc[gen_name, "p_nom"] == 2.5
        assert (
            self.edisgo.topology.generators_df.loc[gen_name, "type"] == "solar"
        )
        assert np.isclose(
        ...
        storage_name = self.edisgo.add_component(
            "StorageUnit", bus="Testbus", p_nom=3.1
        )
        assert storage_name == "StorageUnit_MVGrid_1_1"
        assert len(self.edisgo.topology.storage_units_df) == num_storages + 1
        assert (
            self.edisgo.topology.storage_units_df.loc[storage_name, "bus"]
            == "Testbus"
        )
        assert (
            self.edisgo.topology.storage_units_df.loc[storage_name, "p_nom"]
            == 3.1
        )
        assert np.isclose(
            self.edisgo.timeseries.storage_units_active_power.loc[
                index[0], storage_name
            ],
            3.1,
        )
        assert np.isclose(
            self.edisgo.timeseries.storage_units_active_power.loc[
                index[1], storage_name
            ],
            -3.1,
        )
        assert np.isclose(
            self.edisgo.timeseries.storage_units_reactive_power.loc[
                index[0], storage_name
            ],
            -tan(acos(0.9)) * 3.1,
        )
    ...
    def test_integrate_component(self):
        ...
        assert len(self.edisgo.topology.generators_df) == num_gens + 1
        assert (
            self.edisgo.topology.generators_df.at[comp_name, "subtype"]
            == "misc_sub"
        )
        ...
        ts_active_power = pd.Series(
            data=[1, 2], index=self.edisgo.timeseries.timeindex
        )
        ts_reactive_power = pd.Series(
        ...
        assert (
            self.edisgo.topology.charging_points_df.at[comp_name, "use_case"]
            == "fast"
        )
        # check voltage level
        assert (
        ...
        # check time series
        assert (
            self.edisgo.timeseries.charging_points_active_power.loc[
                :, comp_name
            ].values
            == [1, 2]
        ).all()
        assert (
            self.edisgo.timeseries.charging_points_reactive_power.loc[
                :, comp_name
            ].values
            == [0, 0]
        ).all()
    ...
    def test_aggregate_components(self):
        ...
        assert np.isclose(
            0.2, self.edisgo.topology.charging_points_df.p_nom.sum()
        )
        assert np.isclose(
            0.3,
        ...
        feedin_before = (
            self.edisgo.timeseries.generators_active_power.loc[
                :, ["GeneratorFluctuating_17", "GeneratorFluctuating_18"]
            ]
            .sum()
            .sum()
        )
        ...
        assert (
            self.edisgo.topology.generators_df.at[
                "Generators_Bus_BranchTee_LVGrid_4_2", "p_nom"
            ]
            == 0.065
        )
        assert (
            self.edisgo.timeseries.generators_active_power.loc[
                :, "Generators_Bus_BranchTee_LVGrid_4_2"
            ].sum()
            == feedin_before
        )
    ...
    def test_reduce_memory(self):
        ...
        mem_res_before = self.edisgo.results.pfa_p.memory_usage(
            deep=True
        ).sum()
        ...
        mem_res_with_default = self.edisgo.results.pfa_p.memory_usage(
            deep=True
        ).sum()
        ...
        mem_res_with_default_2 = self.edisgo.results.i_res.memory_usage(
            deep=True
        ).sum()
