...
class TestReinforceMeasures:
    ...
    def test_reinforce_branches_overloading(self):
        ...
        std_line_mv.X_per_km = std_line_mv.L_per_km / 1e3 * omega
        std_line_mv.S_nom = np.sqrt(3) * std_line_mv.U_n * std_line_mv.I_max_th

        std_line_lv = self.edisgo.equipment_data['lv_cables'].loc[
            self.edisgo.config['grid_expansion_standard_equipment'][
                'lv_line']]
        std_line_lv.X_per_km = std_line_lv.L_per_km / 1e3 * omega
        std_line_lv.S_nom = np.sqrt(3) * std_line_lv.U_n * std_line_lv.I_max_th
        # manipulate lines to sdt_line_type
        self.edisgo.topology.lines_df.loc['Line_10017', 'type_info'] = \
            std_line_mv.name
        self.edisgo.topology.lines_df.loc['Line_10017', 'r'] = \
            std_line_mv.R_per_km * self.edisgo.topology.lines_df.loc[
                'Line_10017', 'length']
        self.edisgo.topology.lines_df.loc['Line_10017', 'x'] = \
            std_line_mv.X_per_km * self.edisgo.topology.lines_df.loc[
                'Line_10017', 'length']
        self.edisgo.topology.lines_df.loc['Line_10017', 's_nom'] = \
            std_line_mv.S_nom

        self.edisgo.topology.lines_df.loc['Line_50000002', 'type_info'] = \
            std_line_lv.name
        self.edisgo.topology.lines_df.loc['Line_50000002', 'r'] = \
            std_line_lv.R_per_km * self.edisgo.topology.lines_df.loc[
                'Line_50000002', 'length']
        self.edisgo.topology.lines_df.loc['Line_50000002', 'x'] = \
            std_line_lv.X_per_km * self.edisgo.topology.lines_df.loc[
                'Line_50000002', 'length']
        self.edisgo.topology.lines_df.loc['Line_50000002', 's_nom'] = \
            std_line_lv.S_nom
