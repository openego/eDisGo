...
def add_charging_points_manually(edisgo, num_cps=30, seed=42):
    ...
    if len(lv_buses) < num_cps:
        print(f"  ⚠ Warning: Only {len(lv_buses)} LV buses available, using all")
    ...
    print(f"    - §14a eligible (>4.2 kW): {len(cp_df[cp_df['p_set'] > 0.0042])}")
...
def create_hp_timeseries(edisgo, start_date, end_date, scenario="eGon2035"):
    ...
    # Set data
    edisgo.heat_pump.heat_demand_df = pd.DataFrame(heat_demand_data, index=timeindex)
    edisgo.heat_pump.cop_df = pd.DataFrame(cop_data, index=timeindex)
    
    print(f"  ✓ Created time series:")
    print(f"    Heat demand range: {edisgo.heat_pump.heat_demand_df.min().min():.6f} - {edisgo.heat_pump.heat_demand_df.max().max():.6f} MW")
    print(f"    COP range: {edisgo.heat_pump.cop_df.min().min():.2f} - {edisgo.heat_pump.cop_df.max().max():.2f}")
...
def create_cp_timeseries(edisgo, scenario="eGon2035"):
    ...
    print(f"  ✓ Created time series:")
    print(f"    Load range: {cp_load_df.min().min():.6f} - {cp_load_df.max().max():.6f} MW")
...
def setup_edisgo(grid_path, start_date, end_date, scenario="eGon2035", num_hps=50, num_cps=30):
    ...
    if len(generators) > 0:
        # Active power: use p_nom for all timesteps
        gen_active = pd.DataFrame(
            data=generators['p_nom'].values.reshape(1, -1).repeat(num_timesteps, axis=0),
            columns=generators.index,
            index=hp_timeindex
        )
        edisgo.timeseries._generators_active_power = gen_active
        
        # Reactive power: zeros
    ...
    other_loads = edisgo.topology.loads_df[edisgo.topology.loads_df['type'] != 'heat_pump']
    if len(other_loads) > 0:
        # Active power: use p_set for all timesteps
        load_active = pd.DataFrame(
            data=other_loads['p_set'].values.reshape(1, -1).repeat(num_timesteps, axis=0),
            columns=other_loads.index,
            index=hp_timeindex
        )
        edisgo.timeseries._loads_active_power = load_active
        ...
        edisgo.timeseries._loads_reactive_power = load_reactive
    ...
    if not hasattr(edisgo.timeseries, '_loads_active_power') or edisgo.timeseries._loads_active_power is None:
        edisgo.timeseries._loads_active_power = pd.DataFrame(index=hp_timeindex)
    
    if not hasattr(edisgo.timeseries, '_loads_reactive_power') or edisgo.timeseries._loads_reactive_power is None:
        edisgo.timeseries._loads_reactive_power = pd.DataFrame(index=hp_timeindex)
    
    # Add HP electrical loads
    for hp_name in heat_pumps.index:
        hp_load = edisgo.heat_pump.heat_demand_df[hp_name] / edisgo.heat_pump.cop_df[hp_name]
    ...
    if hasattr(edisgo, 'charging_point_loads'):
        charging_points = edisgo.topology.loads_df[edisgo.topology.loads_df['type'] == 'charging_point']
        for cp_name in charging_points.index:
            edisgo.timeseries._loads_active_power[cp_name] = edisgo.charging_point_loads[cp_name].values
    ...
    print(f"  Power range: {heat_pumps['p_set'].min()*1000:.1f} - {heat_pumps['p_set'].max()*1000:.1f} kW")
    print(f"  §14a eligible (>4.2 kW): {len(heat_pumps[heat_pumps['p_set'] > 0.0042])}")
    
    # Charging point statistics
    charging_points = edisgo.topology.loads_df[edisgo.topology.loads_df['type'] == 'charging_point']
    if len(charging_points) > 0:
        print(f"\n  Charging points: {len(charging_points)}")
        print(f"  Power range: {charging_points['p_set'].min()*1000:.1f} - {charging_points['p_set'].max()*1000:.1f} kW")
        print(f"  §14a eligible (>4.2 kW): {len(charging_points[charging_points['p_set'] > 0.0042])}")
        
        # DEBUG: Verify charging_points_df property works
        cp_via_property = edisgo.topology.charging_points_df
        print(f"  DEBUG: topology.charging_points_df returns {len(cp_via_property)} CPs")
...
def analyze_curtailment_results(edisgo, output_dir="results_14a"):
    ...
    hp_gen_cols = [col for col in edisgo.timeseries.generators_active_power.columns 
                   if 'hp_14a_support' in col]
    cp_gen_cols = [col for col in edisgo.timeseries.generators_active_power.columns 
    ...
    flexible_loads = edisgo.timeseries.loads_active_power[all_flexible_loads.index]
    
    # Separate for detailed analysis
    hp_loads = edisgo.timeseries.loads_active_power[heat_pumps.index] if len(heat_pumps) > 0 else pd.DataFrame()
    cp_loads = edisgo.timeseries.loads_active_power[charging_points.index] if len(charging_points) > 0 else pd.DataFrame()
    ...
    curtailment_percentage = (total_curtailment / total_flexible_load * 100) if total_flexible_load > 0 else 0
    
    flexible_curtailment_total = curtailment.sum()
    curtailed_units = flexible_curtailment_total[flexible_curtailment_total > 0]
    
    # Separate HP and CP curtailment
    hp_curtailment_total = curtailment[hp_gen_cols].sum() if len(hp_gen_cols) > 0 else pd.Series()
    cp_curtailment_total = curtailment[cp_gen_cols].sum() if len(cp_gen_cols) > 0 else pd.Series()
    curtailed_hps = hp_curtailment_total[hp_curtailment_total > 0] if len(hp_curtailment_total) > 0 else pd.Series()
    curtailed_cps = cp_curtailment_total[cp_curtailment_total > 0] if len(cp_curtailment_total) > 0 else pd.Series()
    ...
    results = {
        ...
        'hp_curtailment_data': curtailment[hp_gen_cols] if len(hp_gen_cols) > 0 else pd.DataFrame(),
        'cp_curtailment_data': curtailment[cp_gen_cols] if len(cp_gen_cols) > 0 else pd.DataFrame(),
        'hp_loads': hp_loads,
        'cp_loads': cp_loads,
    ...
    # Print summary
    print(f"\n📈 Summary Statistics:")
    print(f"  Virtual generators: {len(all_gen_cols)} (HPs: {len(hp_gen_cols)}, CPs: {len(cp_gen_cols)})")
    print(f"  Heat pumps curtailed: {len(curtailed_hps)} / {len(heat_pumps)}")
    print(f"  Charging points curtailed: {len(curtailed_cps)} / {len(charging_points)}")
    print(f"  Total curtailment: {total_curtailment:.2f} MWh")
    print(f"  Total flexible load: {total_flexible_load:.2f} MWh (HP: {total_hp_load:.2f}, CP: {total_cp_load:.2f})")
    print(f"  Curtailment ratio: {curtailment_percentage:.2f}%")
    print(f"  Max curtailment: {curtailment.max().max():.4f} MW")
    print(f"  Max total curtailment (timestep): {max_curtailment_value:.4f} MW at {max_curtailment_timestep}")
    
    if len(curtailed_hps) > 0:
        print(f"\n  Top 5 curtailed heat pumps:")
        for i, (hp, value) in enumerate(curtailed_hps.sort_values(ascending=False).head().items(), 1):
            hp_name = hp.replace('hp_14a_support_', '')
            print(f"    {i}. {hp_name}: {value:.4f} MWh")
    
    if len(curtailed_cps) > 0:
        print(f"\n  Top 5 curtailed charging points:")
        for i, (cp, value) in enumerate(curtailed_cps.sort_values(ascending=False).head().items(), 1):
            cp_name = cp.replace('cp_14a_support_', '').replace('charging_point_14a_support_', '')
...
def create_plots(results, output_dir="results_14a"):
    ...
    ax.plot(curtailment_per_timestep.index, curtailment_per_timestep.values, 
            'r-', linewidth=1.5, alpha=0.7, marker='o', markersize=3)
    ax.set_xlabel('Time', fontsize=12)
    ax.set_ylabel('Total Curtailment (MW)', fontsize=12)
    ax.set_title(f'§14a Heat Pump & Charging Point Curtailment - {num_days} Days', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/01_curtailment_timeseries.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Plot 2: Daily curtailment
    print("2. Creating daily curtailment plot...")
    fig, ax = plt.subplots(figsize=(16, 5))
    ax.bar(curtailment_daily.index, curtailment_daily.values, width=1, color='red', alpha=0.7)
    ax.set_xlabel('Date', fontsize=12)
    ax.set_ylabel('Daily Curtailment (MWh)', fontsize=12)
    ax.set_title('§14a Daily Curtailment', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(f"{output_dir}/02_curtailment_daily.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/03_curtailment_monthly.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if len(all_curtailed_list) > 0:
        ...
        unit_names = [name.replace('hp_14a_support_', 'HP: ').replace('cp_14a_support_', 'CP: ').replace('charging_point_14a_support_', 'CP: ') for name in top10.index]
        colors = ['blue' if 'HP:' in name else 'green' for name in unit_names]
        ax.barh(unit_names, top10.values, color=colors, alpha=0.7)
    else:
        ax.text(0.5, 0.5, 'No curtailed units found', ha='center', va='center', transform=ax.transAxes)
    
    ax.set_xlabel('Total Curtailment (MWh)', fontsize=12)
    ax.set_ylabel('Unit', fontsize=12)
    ax.set_title('Top 10 Curtailed Units (Heat Pumps & Charging Points)', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='x')
    ax.invert_yaxis()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/04_top10_curtailed_hps.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Plot 5: Curtailment distribution (histogram)
    print("5. Creating curtailment distribution plot...")
    fig, ax = plt.subplots(figsize=(10, 6))
    curtailment_nonzero = curtailment_per_timestep[curtailment_per_timestep > 0]
    ax.hist(curtailment_nonzero.values, bins=50, color='red', alpha=0.7, edgecolor='black')
    ax.set_xlabel('Curtailment (MW)', fontsize=12)
    ax.set_ylabel('Frequency', fontsize=12)
    ax.set_title('Distribution of Non-Zero Curtailment Events', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(f"{output_dir}/05_curtailment_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if hp_original_name in hp_loads.columns:
        ...
        ax1 = axes[0]
        ax1.plot(original_load.index, original_load.values, 'b-', linewidth=0.5, 
                label='Original Load', alpha=0.7)
        ax1.plot(net_load.index, net_load.values, 'g-', linewidth=0.5, 
                label='Net Load (after curtailment)', alpha=0.7)
        ax1.axhline(y=0.0042, color='orange', linestyle='--', linewidth=1, 
                   label='§14a Minimum (4.2 kW)')
        ax1.set_xlabel('Time', fontsize=12)
        ax1.set_ylabel('Power (MW)', fontsize=12)
        ax1.set_title(f'{hp_original_name} - Full Year Profile', fontsize=14, fontweight='bold')
        ax1.grid(True, alpha=0.3)
        ax1.legend()
        
        # Sample week (first week with curtailment)
        curtailment_weeks = curtailment_power.resample('W').sum()
        first_curtailment_week = curtailment_weeks[curtailment_weeks > 0].index[0]
        week_start = first_curtailment_week
        week_end = week_start + pd.Timedelta(days=7)
        
        ax2 = axes[1]
        week_mask = (original_load.index >= week_start) & (original_load.index < week_end)
        ax2.plot(original_load.index[week_mask], original_load.values[week_mask], 
                'b-', marker='o', linewidth=2, label='Original Load', markersize=3)
        ax2.plot(net_load.index[week_mask], net_load.values[week_mask], 
                'g-', marker='s', linewidth=2, label='Net Load', markersize=3)
        ax2.fill_between(original_load.index[week_mask], 
                        net_load.values[week_mask], 
                        original_load.values[week_mask], 
                        alpha=0.3, color='red', label='Curtailed Power')
        ax2.axhline(y=0.0042, color='orange', linestyle='--', linewidth=2, 
                   label='§14a Minimum (4.2 kW)')
        ax2.set_xlabel('Time', fontsize=12)
        ax2.set_ylabel('Power (MW)', fontsize=12)
        ax2.set_title(f'{hp_original_name} - Sample Week with Curtailment', 
                     fontsize=14, fontweight='bold')
        ax2.grid(True, alpha=0.3)
        ax2.legend()
        
        plt.tight_layout()
        plt.savefig(f"{output_dir}/detailed_hp_profiles.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if 'curtailed_cps' in results and len(results['curtailed_cps']) > 0:
        ...
        if cp_original_name in cp_loads.columns:
            ...
            ax1.set_xlabel('Time', fontsize=12)
            ax1.set_ylabel('Power (MW)', fontsize=12)
            ax1.set_title(f'{cp_original_name} - Full Period Profile', fontsize=14, fontweight='bold')
            ax1.grid(True, alpha=0.3)
            ax1.legend()
            
            # Sample week with curtailment
            curtailment_weeks = curtailment_power.resample('W').sum()
            first_curtailment_week = curtailment_weeks[curtailment_weeks > 0].index[0] if any(curtailment_weeks > 0) else curtailment_weeks.index[0]
            week_start = first_curtailment_week
            week_end = week_start + pd.Timedelta(days=7)
            ...
            ax2.fill_between(original_load.index[week_mask], 
                            net_load.values[week_mask], 
                            original_load.values[week_mask], 
                            alpha=0.3, color='orange', label='Curtailed Power')
            ax2.axhline(y=0.0042, color='orange', linestyle='--', linewidth=2, 
                       label='§14a Minimum (4.2 kW)')
            ax2.set_xlabel('Time', fontsize=12)
            ax2.set_ylabel('Power (MW)', fontsize=12)
            ax2.set_title(f'{cp_original_name} - Sample Week', fontsize=14, fontweight='bold')
            ax2.grid(True, alpha=0.3)
            ax2.legend()
            
            plt.tight_layout()
            plt.savefig(f"{output_dir}/detailed_cp_profiles.png", dpi=300, bbox_inches='tight')
            plt.close()
