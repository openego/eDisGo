.. examples:

Examples
=============

This section provides examples on how to use the eDisGo package. 
It includes a `basic example <https://github.com/openego/eDisGo/blob/dev/examples/edisgo_simple_example.ipynb>`_, 
an `electromobility example <https://github.com/openego/eDisGo/blob/dev/examples/edisgo_simple_emob_example.ipynb>`_ 
and a `plot example <https://github.com/openego/eDisGo/blob/dev/examples/plotting_example.ipynb>`_.


Basic example
------------------------------
.. toctree::

    ../examples/edisgo_simple_example.ipynb
    

    
Electromobility example
------------------------------    
.. toctree:: 

    ../examples/edisgo_simple_emob_example.ipynb
    
    
Plot example
------------------------------  
.. toctree::

    ../examples/plot_example.ipynb
    
    
