...
* Reworked :meth:`~.network.electromobility.Electromobility.get_flexibility_bands`
  to accumulate the energy and power bands in numpy arrays instead of per-process
  pandas label-slice assignments, fixing an intermittent segfault and greatly
  speeding it up on large grids; band values are numerically unchanged
